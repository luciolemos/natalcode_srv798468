...
class AdminLibraryCategoryFormPageAction extends AbstractAdminLibraryAction
{
    private const FLASH_KEY_PREFIX = 'admin_library_category_form_';
    ...
    public function __invoke(Request $request, Response $response): Response
    {
        ...
        if ($isEdit) {
            $existingCategory = $this->libraryRepository->findCategoryByIdForAdmin($categoryId);
        ...
        try {
            if ($isEdit) {
                $this->libraryRepository->updateCategory($categoryId, $payload);
                $this->storeSessionFlash(AdminLibraryCategoryListPageAction::FLASH_KEY, [
                    'status' => 'updated',
                ]);
            ...
            $newId = $this->libraryRepository->createCategory($payload);
            ...
            $this->logger->warning('Falha ao salvar categoria da biblioteca no admin.', [
    ...
    private function resolveFormPath(?int $categoryId): string
    {
        return ($categoryId !== null && $categoryId > 0)
            ? '/painel/biblioteca/categorias/' . $categoryId . '/editar'
            : '/painel/biblioteca/categorias/nova';
    }
    ...
    private function renderForm(
        ...
        return $this->renderPage($response, 'pages/admin-library-category-form.twig', [
            'library_category_form' => $form,
            'library_category_form_errors' => $errors,
            'library_category_form_is_edit' => $isEdit,
            'library_category_id' => $existingCategory['id'] ?? null,
            'page_title' => ($isEdit ? 'Editar categoria de livros' : 'Nova categoria de livros') . ' | Dashboard',
            'page_url' => 'https://cedern.org/painel/biblioteca/categorias',
            'page_description' => 'Formulário do dashboard para categorias da biblioteca.',
        ]);
    }
}
