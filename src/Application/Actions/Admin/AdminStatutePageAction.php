...
class AdminStatutePageAction extends AbstractPageAction
{
    private const DOCUMENT_SLUG = 'estatuto';
    ...
    public function __invoke(Request $request, Response $response): Response
    {
        ...
        if (empty($errors)) {
            try {
                ...
                $this->logger->warning('Falha ao salvar estatuto no painel.', [
                    'error' => $exception->getMessage(),
                ]);

                $errors[] = 'Erro ao salvar o estatuto. Verifique a conexão e tente novamente.';
            }
        }
    ...
    private function validatePayload(array $payload): array
    {
        ...
        if ($payload['title'] === '') {
            $errors[] = 'Informe o título da página do estatuto.';
        }

        if ($payload['body'] === '') {
            $errors[] = 'Informe o conteúdo do estatuto.';
        }
     ...
     */
    private function loadStatuteRecord(): array
    {
        $defaults = $this->loadDefaultStatute();
        $record = [
        ...
        try {
            ...
            $this->logger->warning('Falha ao carregar estatuto no painel.', [
                'error' => $exception->getMessage(),
            ]);
        }
