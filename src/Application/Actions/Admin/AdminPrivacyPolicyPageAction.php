...
class AdminPrivacyPolicyPageAction extends AbstractPageAction
{
    private const DOCUMENT_SLUG = 'politica-de-privacidade';
    private const FORM_ACTION_PATH = '/painel/institucional/politica-de-privacidade';
    private const PUBLIC_PAGE_PATH = '/politica-de-privacidade';
    private const DEFAULT_CONTENT_PATH = '/app/content/privacy-policy.php';
    private const FLASH_KEY = 'admin_document_privacy_policy';
    ...
    public function __invoke(Request $request, Response $response): Response
    {
        ...
        if (empty($errors)) {
            try {
                ...
                $errors[] = 'Não foi possível salvar a Política de Privacidade. Tente novamente.';
            } catch (\Throwable $exception) {
                $this->logger->warning('Falha ao salvar política de privacidade no painel.', [
                    'error' => $exception->getMessage(),
                ]);

                $errors[] = 'Erro ao salvar a Política de Privacidade. Verifique a conexão e tente novamente.';
            }
        }
    ...
    private function validatePayload(array $payload): array
    {
        ...
        if ($payload['title'] === '') {
            $errors[] = 'Informe o título da página da Política de Privacidade.';
        }

        if ($payload['body'] === '') {
            $errors[] = 'Informe o conteúdo da Política de Privacidade.';
        }
    ...
    private function loadDocumentRecord(): array
    {
        ...
        try {
            ...
            $this->logger->warning('Falha ao carregar política de privacidade no painel.', [
                'error' => $exception->getMessage(),
            ]);
        }
    ...
    private function renderForm(Response $response, string $status, array $record, array $errors): Response
    {
        return $this->renderPage($response, 'pages/admin-institutional-document.twig', [
            ...
            'admin_document_label' => 'Política de Privacidade',
            'admin_document_form_action' => self::FORM_ACTION_PATH,
            'admin_document_public_url' => self::PUBLIC_PAGE_PATH,
            'admin_document_dashboard_title' => 'Política de Privacidade',
